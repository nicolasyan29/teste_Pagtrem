...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO funcionarios (nome, cpf, cargo, email, telefone, cep, rua, bairro, cidade, estado, localizacao) VALUES (:nome,:cpf,:cargo,:email,:telefone,:cep,:rua,:bairro,:cidade,:estado,:local)");
        $stmt->execute([
            ':nome'=>$nome,':cpf'=>$cpf,':cargo'=>$cargo,':email'=>$email,':telefone'=>$telefone,
            ':cep'=>$cep,':rua'=>$rua,':bairro'=>$bairro,':cidade'=>$cidade,':estado'=>$estado,':local'=>$local
        ]);
...
?>
<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Novo Funcionário</title><link rel="stylesheet" href="../assets/style.css"></head><body>
  <div class="app-shell">
